...
# Single-model checkpoint helpers (shadow the two-model versions in utils.py,
# which are for the image+text finetuning stage). The 'model_state_dict' key
# here is what main_finetune.py's --finetune_adapter loader expects.
def save_checkpoint(model, optimizer, scaler, epoch, best_test_loss, avg_test_loss, test_auc, checkpoint_dir, name):
    checkpoint = {
        'epoch': epoch + 1,
    ...
    torch.save(checkpoint, os.path.join(checkpoint_dir, f"{name}.pt"))
...
def load_checkpoint(checkpoint_path, model, optimizer, scaler):
    ...
    return checkpoint['epoch'], checkpoint['best_test_loss'], checkpoint['test_auc']
...
def main(rank, world_size, args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        avg_train_loss = pretrain_one_epoch(model, train_dataloader, criterion, optimizer, scaler, device, rank, epoch, args.num_epochs, args)
