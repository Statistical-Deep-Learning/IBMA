...
class MedicalReportDataset(Dataset):
    def __init__(self, json_file, label_names=CHEXPERT_LABELS):
    ...
    def __getitem__(self, idx):
        ...
        # Extract the labels (one entry per configured label name)
        labels = torch.tensor(
            [item[name] for name in self.label_names], dtype=torch.float32
        )
...
class CheXpertVLMDataset(Dataset):
	def __init__(self, json_file, embedding_file, image_root, image_centers, image_anti_centers, embedding_centers, embedding_anti_centers, transform, mode='train', label_names=CHEXPERT_LABELS):
		self.image_root = image_root
		self.mode = mode
		self.transform = transform
		self.label_names = list(label_names)
		self.num_classes = len(self.label_names)
		# 'train' needs embeddings + centroids (to build per-sample scores);
		# 'bootstrap' needs embeddings only (raw inputs for centroid estimation);
		# 'test' needs neither.
		if mode in ('train', 'bootstrap'):
	...
	def __getitem__(self, idx):
		...
		if self.mode == 'bootstrap':
			# Raw inputs only — used to estimate input-space centroids.
			embedding = self.embedding_dict[item["Path"]]['embedding']
			return image, embedding, labels
...
def build_dataset_chest_xray(split, image_centers, image_anti_centers, embedding_centers, embedding_anti_centers, args):
	# 'bootstrap' reads the train file with eval (non-augmented) transforms so the
	# input-space centroids are stable; it carries embeddings but no scores.
	is_train = (split == 'train')
	transform = build_transform(is_train, args)
	if split == 'test':
		file, mode = args.test_file, 'test'
	else:
		file, mode = args.train_file, split  # 'train' or 'bootstrap'
	embedding_file = args.train_embedding_file
	label_names = getattr(args, 'label_names', CHEXPERT_LABELS)
	dataset = CheXpertVLMDataset(json_file=file, embedding_file=embedding_file, image_root=args.data_path, image_centers=image_centers,
							  image_anti_centers=image_anti_centers, embedding_centers=embedding_centers, embedding_anti_centers=embedding_anti_centers, transform=transform, mode=mode, label_names=label_names)
	return dataset