...
from losses import CrossModalIB
...
def train_one_epoch(image_model, text_model, dataloader, criterion, optimizer, scaler, device, rank, epoch, num_epochs, image_feature_centroids, image_feature_anti_centroids, text_feature_centroids, text_feature_anti_centroids, args):
	...
	# Cross-modal alignment via the IB upper bound (IBB). Parameter-free and
	# class-count-agnostic — replaces the per-class unrolled IB computation.
	cross_modal_ib = CrossModalIB(beta=getattr(args, 'ib_beta', 1.0))
	...
	for batch_idx, (input, embedding, targets, input_score, embedding_score) in progress_bar:
		...
		# IBB alignment is computed in float32 (handled inside CrossModalIB),
		# outside autocast, for numerically stable log/softmax terms.
		image_ib_loss, text_ib_loss = cross_modal_ib(
			image_features, text_features,
			input_score, embedding_score,
			image_feature_centroids, image_feature_anti_centroids,
			text_feature_centroids, text_feature_anti_centroids,
		)

		loss = ce_loss + args.image_ib_weight * image_ib_loss + args.text_ib_weight * text_ib_loss
		...
		image_IB.append(image_ib_loss.item())
		text_IB.append(text_ib_loss.item())
		...
		if rank == 0:
			progress_bar.set_postfix(loss=loss.item(), ce_loss=ce_loss.item(), image_IB_loss=image_ib_loss.item(), text_IB_loss=text_ib_loss.item())

	k = len(epoch_loss)
	return sum(epoch_loss) / k, sum(bc_loss) / k, sum(image_IB) / k, sum(text_IB) / k

def pretrain_one_epoch(model, dataloader, criterion, optimizer, scaler, device, rank, epoch, num_epochs, args):
	"""Single-model classification epoch (e.g. text-adapter pretraining).

	No cross-modal IB term — just the task loss. The dataloader yields
	``(features, targets)`` and the model returns ``(logits, features)``.
	"""
	model.train()
	epoch_loss = []
	optimizer.zero_grad()
	...
	for batch_idx, (inputs, targets) in progress_bar:
		...
		scaler(loss, optimizer, clip_grad=None, parameters=model.parameters(), create_graph=False, update_grad=True)
...
# NOTE: the per-class IB computation that used to live here (``get_IB`` /
# ``get_IB_Loss``) has been replaced by the parameter-free, class-count-agnostic
# IB upper bound in ``losses.CrossModalIB`` (a.k.a. IBB). See ``train_one_epoch``.
...
def get_input_centroids(dataloader, device, rank, args):
	...
	# Raw-input dimensionalities for the two modality views. Derived from args so
	# the centroid buffers generalize beyond the medical (image/text) setup:
	#   image_size  = H * W * C  (flattened raw image)
	#   embedding_size = raw modality-B feature dim (e.g. LLM text embedding)
	image_size = getattr(args, 'image_raw_dim', args.input_size * args.input_size * 3)
	embedding_size = getattr(args, 'text_raw_dim', args.input_dim)
...
def compute_centroids(features, labels, num_classes):
	...
	for cls in range(num_classes):
		centroids, anti_centroids, counts, anti_counts = get_counts(cls, features, labels, centroids, anti_centroids, counts, anti_counts)
