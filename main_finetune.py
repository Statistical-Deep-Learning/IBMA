...
from config import chexpert_config
...
def main(rank, world_size, args):
	...
	# Centroids in raw-input space anchor the IB bound. Compute them once from a
	# "bootstrap" loader yielding raw (image, embedding, label) tuples — no
	# per-sample scores yet, since those scores depend on the centroids we are
	# about to compute. Centroids are kept on CPU so the dataset workers can use
	# them when building per-sample input scores.
	if all(os.path.exists(path) for path in paths.values()):
		...
		# Calculate centroids and anti-centroids from a raw-input bootstrap loader
		bootstrap_dataset = build_dataset_chest_xray('bootstrap', None, None, None, None, args)
		bootstrap_sampler = DistributedSampler(bootstrap_dataset, num_replicas=world_size, rank=rank, shuffle=False)
		bootstrap_dataloader = DataLoader(bootstrap_dataset, batch_size=args.batch_size, sampler=bootstrap_sampler, num_workers=4, pin_memory=True, drop_last=False)

		image_centers, image_anti_centers, embedding_centers, embedding_anti_centers = get_input_centroids(bootstrap_dataloader, device, rank, args)
		image_centers, image_anti_centers = image_centers.cpu(), image_anti_centers.cpu()
		embedding_centers, embedding_anti_centers = embedding_centers.cpu(), embedding_anti_centers.cpu()

		# Save once (rank 0); all ranks already hold identical centroids in memory.
		if rank == 0:
...
if __name__ == "__main__":
	...
	parser.add_argument('--ib_beta', default=1.0, type=float, help='Softness of the IBB cluster-assignment softmax')

	args = parser.parse_args()

	# Resolve the use-case config. The medical CheXpert setup is one use case;
	# swapping in another config() here (or selecting by a flag) repurposes the
	# whole pipeline without touching the engine or the IBB alignment loss.
	cfg = chexpert_config(
		input_size=args.input_size,
		text_embed_dim=args.input_dim,
		feature_dim=args.output_dim,
		ib_weight_a=args.image_ib_weight,
		ib_weight_b=args.text_ib_weight,
		ib_beta=args.ib_beta,
	)
	# Inject the config-derived values the dataloader / engine read off `args`.
	args.label_names = cfg.label_names
	args.num_classes = cfg.num_classes
	args.image_raw_dim = cfg.modality_a.raw_dim
	args.text_raw_dim = cfg.modality_b.raw_dim
